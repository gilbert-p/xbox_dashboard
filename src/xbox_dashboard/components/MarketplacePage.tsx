...
import { MarketplacePageProps, SpotlightKeyIdentifier, DashboardDataItem, SpotlightCategories, CustomRootVars } from '../../custom_types/utilityTypes';


const MarketplacePage : React.FC<MarketplacePageProps> = ({mockDbData, slideBladesAway, slideBladesBack}) => {
    
  const marketplaceDatabase = process.env.REACT_APP_DATABASE_MARKETPLACE;

  const [spotlightData, setSpotlightData] = useState<SpotlightKeyIdentifier | null>(null);
  ...
  interface RenderRowItemSkeletonProps {
    children: React.ReactNode;
    count: number;
  }

  function RenderRowItemSkeleton({ children, count }: RenderRowItemSkeletonProps): JSX.Element {
  ...
  function RenderSpotlightRowItem(): React.ReactNode {
    if (!spotlightData) {
      return (
    ...
    return spotlightData[spotlightCategoryTitle].map((category: DashboardDataItem, index: number) => {
      switch (spotlightCategoryTitle as SpotlightCategories) {
        case 'games':
          return (
  ...
  return (
     <>
        <div id={marketplaceStyles["marketplaceContextContainer"]} className={pageGridStyles.outerContextContainer} style={{"--z-depth": `${current_context_index === 0 ? 1 : -1}`} as CustomRootVars}>
